...
describe('production hardening checks', () => {
  ...
  it('publishes Google and AI discovery files for the Relaxed Menu subdomain', () => {
    ...
    expect(layout).toContain('QR menu')
    expect(layout).toContain('restaurant menu')
    expect(layout).toContain('food menu')
    expect(layout).toContain('online food menu')
    expect(layout).toContain('restaurant digital menu')
    ...
    expect(llms).toContain('restaurant menu')
    expect(llms).toContain('food menu')
    expect(llms).toContain('QR menu Bahrain')
  ...
  it('shows both monthly 3 BHD and yearly 30 BHD Pro pricing on the public landing page in both languages', () => {
    const landing = read('src/app/page.tsx')
    const ar = read('messages/ar.json')
    const en = read('messages/en.json')

    expect(landing).toContain("3")
    expect(landing).toContain("BD / mo")
    expect(landing).toContain("30")
    expect(landing).toContain("BD / yr")
    expect(landing).toContain("proYearlySavings")
    expect(en).toContain('3 BD')
    expect(en).toContain('30 BD')
    expect(en).toContain('Save 2 months')
    expect(ar).toContain('٣ د.ب')
